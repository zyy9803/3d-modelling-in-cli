...
export function createSelectionContext(input: SelectionContextInput): DownloadedSelectionContext {
  return {
    version: 1,
    model: {
      file: input.fileName,
...
export function createChatSelectionContext(input: ChatSelectionContextInput): SelectionContextPayload {
  return {
    mode: input.selection.mode,
    triangleIds: input.selection.triangleIds,
    ...(input.selection.screenRect ? { screenRect: input.selection.screenRect } : {}),
    components: input.components,
  };
}
