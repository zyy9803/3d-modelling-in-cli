...
export class OrientationGizmoOverlay {
  ...
  private buildAxes(): void {
    for (const config of GIZMO_AXIS_CONFIGS) {
      ...
      const points = [direction.clone().multiplyScalar(AXIS_OFFSET), direction.clone().multiplyScalar(AXIS_LENGTH)];
...
export function getGizmoCameraPosition(viewDirection: Vector3): Vector3 {
  const direction = viewDirection.lengthSq() > 0 ? viewDirection.clone().normalize() : FALLBACK_GIZMO_DIRECTION.clone();
